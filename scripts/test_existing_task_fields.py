...
async def test_existing_task_fields():
    ...
    print("🔍 VERIFICANDO CAMPOS PERSONALIZADOS DE TAREAS EXISTENTES")
    print("=" * 60)
            ...
            async with session.get(f"{base_url}/api/v1/tasks") as response:
                ...
                if response.status == 200:
                    ...
                    for task in tasks:
                        ...
                        print(f"   📧 Campos personalizados: {custom_fields}")
                        
                        # Si es la tarea "Partido =", mostrar más detalles
                        if "Partido" in task_name:
                            print(f"   🎯 ¡ENCONTRADA LA TAREA CON CAMPOS PERSONALIZADOS!")
                            print(f"   📧 Email: {custom_fields.get('Email', 'No encontrado')}")
                            print(f"   📱 Celular: {custom_fields.get('Celular', 'No encontrado')}")
                            
                            # Intentar obtener más detalles de esta tarea específica
                            task_id = task.get("id")
                            if task_id:
                                print(f"\n🔍 Obteniendo detalles completos de la tarea {task_id}...")
                                async with session.get(f"{base_url}/api/v1/tasks/{task_id}") as detail_response:
                                    if detail_response.status == 200:
                                        ...
                                        print(f"   📝 Nombre: {detail_data.get('name')}")
