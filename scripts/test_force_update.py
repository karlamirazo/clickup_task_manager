#!/usr/bin/env python3
"""
Script para forzar y probar la actualizacion post-creacion
"""
...
async def test_force_update():
    """Forzar y probar la actualizacion post-creacion"""
    
    print("ğŸ§ª FORZANDO Y PROBANDO ACTUALIZACION POST-CREACION")
    print("=" * 70)
    
    # URL de la API
    base_url = "https://clickuptaskmanager-production.up.railway.app"
    
    # Datos de prueba especificos para forzar la actualizacion
    task_data = {
        "name": f"FORZAR ACTUALIZACION - {datetime.now().strftime('%H:%M:%S')}",
        "description": "Tarea para forzar la actualizacion post-creacion",
        ...
        "custom_fields": {
            "Email": "forzar.actualizacion@test.com",
    ...
    try:
        async with aiohttp.ClientSession() as session:
            print("ğŸ“¤ Creando tarea para forzar actualizacion...")
            ...
            async with session.post(
                ...
                if response.status == 201:
                    ...
                    print(f"\nâ�³ Esperando 20 segundos para actualizacion post-creacion...")
...
if __name__ == "__main__":
    task_id = asyncio.run(test_force_update())
    if task_id:
