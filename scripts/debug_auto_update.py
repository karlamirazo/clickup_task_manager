#!/usr/bin/env python3
"""
Script para debuggear la actualización automática de campos personalizados
"""
...
async def debug_auto_update():
    """Debuggear la actualización automática"""
    
    print("🔍 DEBUGGEANDO ACTUALIZACIÓN AUTOMÁTICA")
    print("=" * 50)
    ...
    task_data = {
        ...
        "description": "Debug de actualización automática",
    ...
    try:
        async with aiohttp.ClientSession() as session:
            print("📤 Creando tarea para debug...")
                ...
                if response.status == 201:
                    ...
                    if task_details:
                        ...
                        if email_field and email_field.get('value') is None:
                            print(f"\n❌ Campo Email está vacío - La actualización automática no funcionó")
                        else:
                            print(f"\n✅ Campo Email tiene valor: {email_field.get('value') if email_field else 'No encontrado'}")
                        
                        if celular_field and celular_field.get('value') is None:
                            print(f"❌ Campo Celular está vacío - La actualización automática no funcionó")
                        else:
                            print(f"✅ Campo Celular tiene valor: {celular_field.get('value') if celular_field else 'No encontrado'}")
